...
greplog() {
	if [ -f .errors ]; then
		grep -Ei 'fail|fatal|cannot' .errors < .errorlog
		awk '/error: /{flag=1} /cc1: /{flag=0} flag' .errors < .errorlog
		printf "${top}\n${mid}%s\n${mid}\n${end}\n\n" "$(sed -e 's/^/\	║	/'.errorlog)"
	else
...
if [ -z "${type}" ] || [ -z "${kname}" ]; then
	final="${device}"-kernel-"${date}".zip
elif [ -z "${type}" ]; then
	final="${kname}"-"${date}".zip
else
	final="${kname}"-"${type}"-"v${version}"-"${date}".zip
fi
case ${arch} in
	"arm")
		img=zImage
...
fmsg "Building DTBs"
run_build dtbs 2> >(tee -a .errors >&2)
fmsg "Building Modules"
run_build modules 2> >(tee -a .errors >&2)
echo ""
run_build modules_install INSTALL_MOD_PATH=modinstall INSTALL_MOD_STRIP=1 2> >(tee -a .errors >&2)
...
if find "${out}"/modinstall/ -name '*.ko' -type f -exec cp '{}' "${ak3}/modules/system/lib/modules/" \; ; then
	fmsg "Modules found"
else
